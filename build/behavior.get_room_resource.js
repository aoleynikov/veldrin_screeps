var behavior = {
  is_fast_provider: function (provider, resource) {
    return provider.structureType == STRUCTURE_CONTAINER || provider.structureType == STRUCTURE_STORAGE || provider.structureType == STRUCTURE_LINK && resource == RESOURCE_ENERGY;
  },
  ...
  get_closest_provider: function (creep) {
    ...
    result = creep.pos.findClosestByRange(FIND_STRUCTURES, {
      filter: s => self.is_fast_provider(s, resource) && self.provider_resource(s, resource) >= (creep.carryCapacity - (creep.carry[resource] || 0)) / 2.0
    });
