...
var strategy = {
  find_container: function (creep) {
    ...
    for (var item of creepLook) if (item.type == 'structure' && item.structure.structureType == STRUCTURE_CONTAINER) if (_.sum(item.structure.store) != item.structure.storeCapacity) if (item.structure.pos.findInRange(creep.memory['find'])) return item.structure;
